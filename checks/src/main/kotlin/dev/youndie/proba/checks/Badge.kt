...
object Badge {
    ...
    fun of(
        ...
        label: String = TOOL,
    ): String = svg(label, message(findings), colour(findings), message(findings))

    fun refusal(
        reason: String,
        label: String = TOOL,
    ): String = svg(label, reason, NEUTRAL, reason)
    ...
    fun message(findings: List<Finding>): String {
        ...
        return when {
            defects > 0 && suspicions > 0 -> "$defects ${plural(
                defects,
                "defect",
            )}, $suspicions ${plural(suspicions, "suspicion")}"
    ...
    private fun colour(findings: List<Finding>): String =
        when {

...
object ComponentMatchesPath : Check {
    ...
    override suspend fun run(context: CheckContext): List<Finding> {
        ...
        val declared = context.publication.component ?: return listOf(
            Finding(
                checkId = id,
        ...
        val disagreement = when {
            declared.isBackReference ->
                if (declared.coordinate.version == asked.version) null
                else "points back at ${declared.coordinate}, a different version from the $asked it is published under"

            declared.coordinate == asked -> null
            else -> "calls itself ${declared.coordinate}"
        } ?: return emptyList()
        ...
        return listOf(
            Finding(
                ...
                evidence = listOf("path $asked", "component ${declared.coordinate}" + if (declared.isBackReference) " (back-reference)" else ""),
            ),
        )
    }
}

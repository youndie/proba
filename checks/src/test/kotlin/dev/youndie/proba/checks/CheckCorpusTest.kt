...
class CheckCorpusTest {
    @Test
    fun `every case gets the answer it is owed`() =
        runTest {
            val wrong =
                CheckCorpus.cases.mapNotNull { case ->
    ...
    fun `no check is registered without a case on each side`() {
        ...
        val missing =
            Checks.all.mapNotNull { check ->
                val expectations =
                    CheckCorpus.cases
                        .filter { it.checkId == check.id }
                        .map { it.expectation }
                        .toSet()
                val lacking =
                    buildList {

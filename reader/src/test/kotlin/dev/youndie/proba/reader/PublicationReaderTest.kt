...
class PublicationReaderTest {
    private fun read(
        ...
        coordinate: Coordinate = Fixtures.KompotCore,
    ) = PublicationReader(client)

    @Test
    fun `follows the redirector and finds every target`() =
        runTest {
            ...
            val outcome =
                read(Fixtures.serving(*Fixtures.KompotCoreDocuments))
    ...
    @Test
    fun `reads what a consumer compiles against apart from what it runs against`() =
        runTest {
            ...
            val outcome =
                PublicationReader(Fixtures.serving("kompot-standard-jvm-0.27.0.46.module"))
    ...
    @Test
    fun `common code compiles against the metadata variant, which is not an api variant`() =
        runTest {
            ...
            val outcome =
                PublicationReader(Fixtures.serving(*Fixtures.KompotCoreDocuments))
                    .read(Fixtures.KompotCore, Fixtures.Repository)
    ...
    @Test
    fun `a coordinate that is not published is refused rather than reported empty`() =
        runTest {
            val outcome =
                PublicationReader(Fixtures.serving()) // serves nothing
    ...
    @Test
    fun `an artefact published without module metadata is named as such`() =
        runTest {
    ...
    @Test
    fun `a root pointing at a module that is not there says so`() =
        runTest {
            ...
            val outcome =
                PublicationReader(Fixtures.serving("kompot-core-0.27.0.46.module")) // root only
                    .read(Fixtures.KompotCore, Fixtures.Repository)
    ...
    @Test
    fun `metadata that is not module metadata is refused, not silently empty`() =
        runTest {
...
class DocumentationVariantTest {
    @Test
    fun `sources with no platform of their own belong to the target there is`() =
        runTest {
            ...
            val outcome =
                PublicationReader(Fixtures.serving("kompot-client-tck-0.28.0.53.module"))
    ...
    @Test
    fun `a multiplatform publication keeps its sources on the target that names them`() =
        runTest {
            val outcome =
                PublicationReader(Fixtures.serving(*Fixtures.KompotCoreDocuments))
                    .read(Fixtures.KompotCore, Fixtures.Repository)
...
class SnapshotLayoutTest {
    ...
    @Test
    fun `a snapshot is read through the timestamp its metadata names`() =
        runTest {
            ...
            val outcome =
                PublicationReader(snapshotRepository())
    ...
    @Test
    fun `the files of a snapshot are named the way they are actually stored`() =
        runTest {
            val outcome =
                PublicationReader(snapshotRepository())
                    .read(Coordinate("io.github.youndie", "s3-client", "0.1.0-SNAPSHOT"), Fixtures.Repository)

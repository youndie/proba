...
fun main(args: Array<String>) {
    ...
    val outcome = HttpClient(CIO).use { http ->
...
private fun report(outcome: ReadOutcome, repository: MavenRepository): Int = when (outcome) {

    is ReadOutcome.Read -> {
        val publication = outcome.publication
        println("${publication.coordinate}")
        println("  read from ${repository.baseUrl} — ${publication.documents.size} document(s), ${publication.targets.size} target(s)")
    ...
    is ReadOutcome.NotFound -> {
        ...
        outcome.tried.forEach { println("  tried ${it.url} → ${if (it.status == 0) "no answer" else it.status.toString()}") }
        2
    }
...
private fun line(label: String, variant: Variant?) {
    if (variant == null) {

...
class ServerTest {
    ...
    @Test
    fun `the root of the screen carries its type`() =
        withFixtures(*Fixtures.KompotCoreDocuments) { client ->
            ...
            val body =
                client
                    .get(
                        "/report/io.github.youndie/kompot-core/0.27.0.46?repo=https://repo.example/maven2",
                    ).bodyAsText()
    ...
    @Test
    fun `the screen names only tokens, never a colour`() =
        withFixtures(*Fixtures.KompotCoreDocuments) { client ->
            val body =
                client
                    .get(
    ...
    @Test
    fun `a coordinate that is not published gets a screen saying so`() =
        withFixtures { client ->
    ...
    @Test
    fun `the report is at a permanent address`() =
        withFixtures(*Fixtures.KompotCoreDocuments) { client ->

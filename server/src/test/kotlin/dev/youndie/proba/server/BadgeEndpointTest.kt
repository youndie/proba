...
class BadgeEndpointTest {
    @Test
    fun `the endpoint serves an svg and asks to be cached`() =
        testApplication {
            application { proba(Fixtures.serving(*Fixtures.KompotCoreDocuments)) }

            val response =
                client.get(
                    "/badge/io.github.youndie/kompot-core/0.27.0.46.svg?repo=https://repo.example/maven2",
                )
    ...
    @Test
    fun `a coordinate that is not published says so rather than looking clean`() =
        testApplication {
